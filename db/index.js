...
const client = new Client({
  connectionString:
    process.env.DATABASE_URL || "https://localhost:5432/juiceboxdev",
  ssl:
 ...
 * USER Methods
 */
async function createBook({ title, author, description, active }) {
  try {
    const {
      rows: [book],
    } = await client.query(
      `
      INSERT INTO books(title, author, description, active) 
      VALUES($1, $2, $3, $4) 
      RETURNING *;
    `,
      [title, author, description, active]
    );
